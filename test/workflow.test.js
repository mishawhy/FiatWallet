...
contract('Passport', function ([
  ...
  describe('Issue passport', function () {
    it('should check if is provider', async function () {
      const isIssuer = await this.passport.isClient(issuer)
    ...
    it('should owner issue own passport', async function () {
      ...
      const type = 3
      const tx = await this.passport.issue(owner, pass, type, {
        from: owner
      })
    ...
    it('should provider issue passport to client1', async function () {
      const pass = await web3.utils.toHex('Client14')
      const type = 1
      const tx = await this.passport.issuePass(client2, pass, type, {
        from: provider
      })
    ...
    it('should set passport fee', async function () {
      const pass = await this.passport.getPass(client1)
