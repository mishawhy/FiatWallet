<template>
    <div @keyup.enter="sign">
        <div class="auth-box">
            <div
                class="connect-logo"
                :style="{
                    backgroundImage: `url(${logo})`,
                    backgroundRepeat: 'no-repeat',
                    backgroundSize: 'cover',
                    backgroundPosition: 'center',
                    backgroundColor: '#fff'
                }"
            ></div>

            <div class="title">{{ name }}</div>
            <div v-if="status == null">
                <i class="far fa-arrow-alt-circle-up auth-icon"></i>

                <div class="auth-text">
                    Please login using your Ethereum wallet {{status}}
                </div>
                <div class="loading" v-if="status === null">
                    <vue-element-loading
                        :active="status == null"
                        spinner="ring"
                        :color="loaderColor"
                    />
                </div>
            </div>
            <div v-if="status === 'ready'">
                <i class="far fa-arrow-alt-circle-up auth-icon"></i>
                <div class="user">
                    <div ref="headerlogo" class="logo"></div>
                    <div class="user-name bold">{{ address | shortAddress }}</div>
                </div>
                <div class="auth-text">
                    Please press Enter to authenticated using your Ethereum wallet
                </div>
            </div>
            <div v-if="status === 'network'">
                <i class="fas fa-exclamation-circle auth-expl-icon"></i>
                <div class="auth-text">
                    Please connect to the Ethereum <font class="bold">{{ network }}</font>
                </div>
            </div>
            <div v-if="status === 'extension' && !isMobile">
                <i class="fas fa-exclamation-circle auth-expl-icon"></i>

                <div class="auth-text">
                    <div
                        class="user-name bold"
                        :style="{
                            marginBottom: `20px`
                        }"
                    >
                        To enter and buy tokens, <br/>an Ethereum wallet is needed.
                    </div>
                    Desktop users can use <span class="bold">MetaMask</span> which is a
                    digital wallet and browser extension for Brave, Firefox, Opera, and
                    Chrome.
                </div>
                <el-button
                    class="button"
                    type="primary"
                    v-if="status === 'extension' && !isMobile"
                    @click="goToMetamask()"
                >
                    Install Metamask
                </el-button>
            </div>

            <div v-if="status === 'extension' && isMobile">
                <i class="fas fa-exclamation-circle auth-expl-icon"></i>
                <div class="auth-text">
                    <div
                        class="user-name bold"
                        :style="{
                            marginBottom: `20px`
                        }"
                    >
                        To enter and buy tokens, <br/>an Ethereum wallet is needed.
                    </div>
                    Mobile users can use the supported ÐApp browsers
                    <span class="bold">coinbase | Wallet</span>, and
                    <span class="bold">Trust Wallet</span>.
                </div>
                <div class="link-box ">
                    <svg
                        height="30"
                        viewBox="0 0 164 28"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                    >
                        <path
                            fill-rule="evenodd"
                            clip-rule="evenodd"
                            d="M49.576172.545441h-2.9453v19.595c1.3398.4742 3.0058.8594 4.9101.8594 4.8809 0 7.7071-2.8458 7.7071-7.0846 0-4.20983-2.8555-6.64028-6.666-6.64028-1.1602 0-2.3809.32621-3.0059.65241V.545441zm-21.0176 1.18585c-.9824 0-1.8457.80039-1.8457 1.80802 0 1.03784.8633 1.8684 1.8457 1.8684.9824 0 1.8457-.83056 1.8457-1.8684 0-1.00763-.8633-1.80802-1.8457-1.80802zm-18.2324 16.30675c-.64258.251-1.32031.3832-2.05273.3832-2.56055 0-4.34766-1.6602-4.34766-4.2689 0-2.7568 1.8457-4.26886 4.22656-4.26886.70117 0 1.38086.11985 2.04102.38318.31836-.77942.74023-1.48746 1.25001-2.11145-1.09376-.58255-2.1797-.88037-3.55665-.88037-3.8711 0-6.93555 2.6377-6.93555 6.8775 0 4.2099 2.91602 6.8476 6.93555 6.8476 1.35156 0 2.58984-.2827 3.77345-.9166-.5371-.6008-.98634-1.2863-1.334-2.0453zm8.084-10.76317c3.957 0 6.8144 2.78684 6.8144 6.87757 0 4.0616-2.8574 6.8475-6.8144 6.8475-3.9278 0-6.7852-2.7859-6.7852-6.8475 0-4.09073 2.8574-6.87757 6.7852-6.87757zm-3.8125 6.87757c0-2.6677 1.6074-4.35792 3.8086-4.35792 2.2637 0 3.8398 1.69022 3.8398 4.35792 0 2.6388-1.5761 4.3279-3.8398 4.3279-2.2012 0-3.8086-1.6891-3.8086-4.3279zm12.5 6.5512h2.9453V7.571011h-2.9453v13.13263zm5.5176-12.302c1.8457-.71139 4.1074-1.1268 6.041-1.1268 3.3633 0 5.5058 1.27481 5.5058 4.9803v8.4486h-2.916v-8.1814c0-1.8973-1.1894-2.57984-2.8281-2.57984-1.041 0-2.082.14823-2.8555.38544v10.3758h-2.9472v-12.3021zm16.9609 9.8125c.5664.1771 1.3105.2661 2.0527.2661 2.709 0 4.6446-1.4819 4.6446-4.5059 0-2.5498-1.8164-4.06165-4.0489-4.06165-1.1601 0-2.0527.29712-2.6484.62332v7.67813zm15.6523-8.38946c1.459 0 2.5293.65127 2.5293 2.07426v1.0978h-1.457c-3.8105 0-6.1016 1.3039-6.1016 3.9715 0 2.965 2.5 4.0026 5.9239 4.0026 1.4863 0 3.2715-.2081 4.5215-.5343v-8.6858c0-3.29008-2.0528-4.4759-4.9395-4.4759-1.6367 0-3.125.41541-4.2851 1.06665v2.54985c1.1015-.62242 2.291-1.06666 3.8085-1.06666zm2.5313 5.12836h-1.25c-2.0527 0-3.3926.5922-3.3926 1.9263 0 1.3638 1.25 1.8972 3.0352 1.8972.4472 0 1.0722-.059 1.6074-.1482v-3.6753zm9.0449 3.6754c-1.3379 0-2.7949-.5634-4.0156-1.482v2.7868c.9824.6524 2.5293 1.0667 4.0762 1.0667 2.8574 0 4.9707-1.2449 4.9707-4.0015 0-2.3717-1.3086-3.4985-4.3164-4.1509-1.5176-.4143-2.0821-.8596-2.0821-1.719 0-.85965.6836-1.48209 1.9649-1.48209 1.3965 0 2.5293.5044 3.6894 1.33379v-2.69759c-1.1015-.68142-2.2929-1.00763-3.7793-1.00763-2.6797 0-4.7031 1.30365-4.7031 3.97242 0 2.2827 1.1621 3.3793 3.8984 4.0017 1.6973.4443 2.4707.8295 2.4707 1.8973 0 1.0376-.8632 1.482-2.1738 1.482zm9.0625-3.8236v.0591c.1778 2.3414 2.2012 3.6163 4.2559 3.6163 1.8144 0 3.125-.4152 4.4336-1.2749v2.5798c-1.1914.8295-2.9473 1.2147-4.6426 1.2147-4.1074 0-6.9043-2.6086-6.9043-6.7584 0-4.17964 2.7383-6.96648 6.3691-6.96648 3.8379 0 5.6543 2.46064 5.6543 6.04698v1.4829h-9.166zm3.4238-5.06921c1.9043 0 2.9161 1.12571 2.9747 3.17201h-6.3086c.3867-1.9862 1.6367-3.17201 3.3339-3.17201z"
                            fill="#1452F5"
                        ></path>
                        <path
                            fill-rule="evenodd"
                            clip-rule="evenodd"
                            d="M105.543.479151c0-.264628.214523-.479151.479151-.479151s.479151.214523.479151.479151V27.0663c0 .2646-.214523.4792-.479151.4792s-.479151-.2146-.479151-.4792V.479151z"
                            fill="#AAB4C0"
                        ></path>
                        <path
                            d="M123.76075 7.60523l-2.94965 11.86278-3.08272-11.86278H116.309l3.57063 13.12198h1.81859l2.83876-11.55353 2.83872 11.55353h1.8186l3.5707-13.12198h-1.3529l-3.1049 11.86278-2.92746-11.86278h-1.61899zM138.3712 15.16032h-1.0867c-2.9496 0-4.7017.99409-4.7017 2.89389 0 2.0324 1.7521 2.8497 4.3691 2.8497.8205 0 1.8629-.1104 2.75-.3534v-6.64937c0-2.16491-1.2641-3.02646-3.2158-3.02646-1.131 0-2.1734.30928-2.994.75109v1.25919c.8871-.46391 1.7299-.77319 2.8388-.77319 1.2419 0 2.0403.64064 2.0403 1.87773v1.17082zm0 4.50659c-.4657.0883-.9536.1325-1.3528.1325-1.7742 0-3.1049-.4639-3.1049-1.7452s1.4637-1.8114 3.4597-1.8114h.998v3.4241zM143.1294 5.70541h-1.3307v15.0218h1.3307V5.70541zM146.7177 5.70541h-1.3307v15.0218h1.3307V5.70541zM156.8928 15.97771v-.90575c0-2.73928-1.619-4.19728-4.0586-4.19728-2.5948 0-4.5908 2.14282-4.5908 5.03673 0 3.1369 2.1291 5.0367 4.9235 5.0367 1.1976 0 2.4396-.2872 3.3045-.9278v-1.3254c-.9758.729-1.9295 1.0824-3.3045 1.0824-1.8851 0-3.5928-1.3917-3.5928-3.7554v-.0442h7.3187zm-7.23-1.10457c.377-1.81146 1.5968-2.82764 3.1049-2.82764 1.619 0 2.7057.81737 2.7944 2.82764h-5.8993zM157.1456 12.22223h1.4859v6.00868c0 1.9882 1.0423 2.7172 2.4395 2.7172.8428 0 1.5081-.1988 2.0848-.486v-1.1708c-.6654.3314-1.1755.5302-1.7965.5302-.9092 0-1.3972-.486-1.3972-1.7231v-5.87618h2.7279v-1.12664h-2.7279V8.75395h-1.3306v2.34164h-1.4859v1.12664z"
                            fill="#202A36"
                        ></path>
                    </svg>
                    <a
                        href="https://itunes.apple.com/app/coinbase-wallet/id1278383455?ls=1&mt=8"
                        target="_blank"
                        title="Get Coinbase wallet for iOS"
                    ><i class="fab fa-apple"></i
                    ></a>
                    <a
                        href="https://play.google.com/store/apps/details?id=org.toshi"
                        target="_blank"
                        title="Get Coinbase wallet for Android"
                    ><i class="fab fa-android"></i
                    ></a>
                </div>

                <div class="link-box trust">
                    <svg
                        data-v-9d6b9a68=""
                        height="50"
                        xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 200 100"
                        class=""
                    >
                        <path
                            data-v-9d6b9a68=""
                            d="M42.9 6.6L45 3.8c-1.3-1-3.1-1-4.4 0l2.3 2.8zm37.3 11l3.6.1c0-1-.4-1.9-1-2.6-.7-.7-1.6-1.1-2.5-1.1v3.6zM42.9 93.9l-2 3c1.2.8 2.8.8 3.9 0l-1.9-3zM5.6 17.6V14c-1 0-1.9.4-2.5 1.1-.7.7-1 1.6-1 2.6l3.5-.1zm35.1-8.1C56.9 22 75.4 21.2 80.2 21.2v-7.1c-5 0-21.2.6-35.2-10.2l-4.3 5.6zm36 8.1c-.3 16.6-1 28.2-2.3 36.9-1.4 8.5-3.3 13.6-5.7 17.2-2.5 3.7-5.6 6-10.2 8.7-4.6 2.8-10.5 5.8-17.7 10.5l3.9 5.9c6.8-4.6 12.5-7.5 17.4-10.3 4.9-2.9 9.2-6.1 12.4-10.8 3.3-4.8 5.4-11.1 6.8-20.2 1.4-9 2.1-21.1 2.4-37.8l-7-.1zM44.8 91c-7.2-4.8-13-7.8-17.6-10.5-4.6-2.7-7.7-5-10.1-8.7-2.4-3.6-4.3-8.7-5.6-17.3-1.3-8.7-2-20.3-2.3-36.9l-7.2.1c.3 16.7 1 28.9 2.4 37.8 1.4 9.1 3.5 15.4 6.8 20.2 3.2 4.7 7.5 7.9 12.4 10.8 4.8 2.9 10.5 5.8 17.4 10.3l3.8-5.8zM5.6 21.2c4.8 0 23.3.8 39.5-11.7l-4.4-5.6c-14 10.9-30.1 10.2-35.1 10.2v7.1zM114.5 29.9h-7.1v19.5h-4.9V29.9h-7.1V26h19.1zM126 40.8h-3.8v8.6h-4.8V26h8.7c2.8 0 4.9.6 6.4 1.9 1.5 1.2 2.3 3 2.3 5.2 0 1.6-.3 2.9-1 4s-1.7 1.9-3.1 2.5l5.1 9.6v.2h-5.2l-4.6-8.6zm-3.9-3.9h3.9c1.2 0 2.2-.3 2.8-.9.7-.6 1-1.5 1-2.6s-.3-2-.9-2.6c-.6-.6-1.6-.9-2.9-.9h-3.9v7zM156.1 26v15.4c0 2.6-.8 4.6-2.4 6.1-1.6 1.5-3.8 2.2-6.6 2.2-2.7 0-4.9-.7-6.5-2.2-1.6-1.4-2.4-3.4-2.5-6V26h4.8v15.5c0 1.5.4 2.7 1.1 3.4.7.7 1.8 1 3.1 1 2.7 0 4.1-1.4 4.1-4.3V26h4.9zM172.2 43.3c0-.9-.3-1.6-1-2.1-.6-.5-1.8-1-3.5-1.5s-3-1.1-4-1.6c-2.7-1.4-4-3.4-4-5.8 0-1.3.4-2.4 1.1-3.4.7-1 1.7-1.8 3.1-2.3 1.3-.6 2.8-.8 4.5-.8 1.7 0 3.2.3 4.5.9 1.3.6 2.3 1.5 3 2.6.7 1.1 1.1 2.4 1.1 3.8h-4.8c0-1.1-.3-1.9-1-2.5-.7-.6-1.6-.9-2.8-.9-1.2 0-2.1.3-2.8.8-.7.5-1 1.1-1 2 0 .8.4 1.4 1.1 1.9.8.5 1.9 1 3.4 1.4 2.7.8 4.7 1.9 6 3.1 1.3 1.2 1.9 2.7 1.9 4.6 0 2-.8 3.6-2.3 4.8-1.5 1.1-3.6 1.7-6.2 1.7-1.8 0-3.4-.3-4.9-1-1.5-.7-2.6-1.6-3.4-2.7-.8-1.1-1.2-2.5-1.2-4h4.8c0 2.6 1.5 3.9 4.6 3.9 1.1 0 2-.2 2.7-.7.8-.8 1.1-1.4 1.1-2.2zM198 29.9h-7.2v19.5H186V29.9h-7.1V26H198z"
                            fill="#3375bb"
                        ></path>
                        <path
                            data-v-9d6b9a68=""
                            fill="#2e91db"
                            d="M104 70.7l.6 2.9.7-2.7 3.9-13.5h1.4l4 13.5.6 2.7.6-2.9 3.3-13.3h1.6L116 75.7h-1.4l-4.2-14.3-.5-1.9-.4 1.9-4.3 14.3h-1.4l-4.7-18.3h1.6zM133 70.6h-8.3l-1.9 5.1h-1.6l6.9-18.3h1.5l6.9 18.3h-1.6l-1.9-5.1zm-7.8-1.4h7.3l-3.7-10-3.6 10zM140.8 74.4h9.1v1.3h-10.6V57.4h1.5zM154.4 74.4h9v1.3h-10.6V57.4h1.6zM176.5 66.9H168v7.5h9.8v1.3h-11.4V57.4h11.4v1.3H168v6.9h8.5zM193.8 58.7h-6.3v17H186v-17h-6.3v-1.3h14.1z"
                        ></path>
                    </svg>

                    <a
                        href="https://itunes.apple.com/us/app/trust-ethereum-wallet/id1288339409"
                        target="_blank"
                        title="Get Trust wallet for iOS"
                    ><i class="fab fa-apple"></i
                    ></a>
                    <a
                        href="https://play.google.com/store/apps/details?id=com.wallet.crypto.trustapp"
                        target="_blank"
                        title="Get Trust wallet for Android"
                    ><i class="fab fa-android"></i
                    ></a>
                </div>
            </div>

            <el-button
                :loading="loading"
                class="button"
                type="primary"
                :disabled="status !== 'ready'"
                v-if="status !== 'extension' && status !== null"
                @click="sign()"
            >
                Enter with Ethereum wallet
            </el-button>
        </div>
    </div>
</template>

<script>
import TextMixin from '@/mixins/text'
import { mapState } from 'vuex'
import VueElementLoading from 'vue-element-loading'
import { LOGO } from '@/constant/general'
import { NETWORKS } from '@/constant/ethereum'
import { generateBlockies } from '@/utils/blockies'

export default {
  name: 'Sign',
  data: function () {
    return {
      loading: false,
      form: {
        method: 'metamask'
      },
      helpDialog: false,
      logo: LOGO,
      /* eslint-disable */
      loaderColor: VUE_APP_MAIN_COLOR,
      network: NETWORKS[VUE_APP_NETWORK],
      name: VUE_APP_NAME,
      /* eslint-enable */
      isMobile: false,
      tryAgain: false,
      timer: ''
    }
  },
  computed: {
    ...mapState({
      status: state => state.enter.status,
      address: state => state.enter.address
    })
  },
  async created () {
    if (window.innerWidth < 1000) {
      this.isMobile = true
    } else {
      this.isMobile = false
    }
    this.timer = setTimeout(this.showTryAgain, 8000)
    await this.$store.dispatch('enter/initWeb3')
  },

  mounted () {
    if (window.innerWidth < 1000) {
      this.isMobile = true
    } else {
      this.isMobile = false
    }
    // const icon = generateBlockies(this.address, {
    //   active: true,
    //   size: 6,
    //   scale: 10
    // })
    // this.$refs.headerlogo.appendChild(icon)
  },
  methods: {
    async sign () {
      this.loading = true
      await this.$store.dispatch('enter/sign')
      this.loading = false
    },
    async tryAgainLogin () {
      this.tryAgain = false
      this.timer = setTimeout(this.showTryAgain, 8000)
      await this.$store.dispatch('enter/initWeb3')
    },
    showTryAgain () {
      this.tryAgain = true
    },
    goToMetamask () {
      window.open('https://metamask.io', '_blank')
    }
  },
  mixins: [TextMixin],
  components: {
    VueElementLoading
  }
}
</script>

<style lang="scss">
    .auth-box {
        text-align: center;
        margin-top: 20px;

        .link-box {
            margin-top: 10px;
            background-color: #f0f0f0;
            border-radius: 5px;
            padding: 20px 10px 20px 15px;
            display: flex;
            justify-content: space-between;
            a {
                color: #555 !important;
                font-size: 30px;
                padding-left: 10px;
                padding-right: 10px
            }
        }

        .trust {
            padding: 10px 10px 10px 15px;
            a {
                display: inline-block;
                -webkit-border-radius: 50%;
                -moz-border-radius: 50%;
                border-radius: 50%;
                box-shadow: 2px 2px 7px 0 rgba(220, 220, 220, 0.50)
            }
        }

        .user-name {
            font-size: 22px;
            margin-top: 15px
        }
        .user {
            margin-top: 20px;
            margin-bottom: 20px
        }

        .loading {
            margin: 30px auto;
            height: 60px;
        }

        .connect-logo {
            width: 120px;
            height: 120px;
            display: inline-block;
            -webkit-border-radius: 50%;
            -moz-border-radius: 50%;
            border-radius: 50%;
        }
        .title {
            text-align: center;
            font-size: 24px;
            margin-top: 5px;
            letter-spacing: 0.4px;
            font-weight: 600;
        }

        .auth-icon {
            display: block;
            margin-top: 30px;
            font-size: 40px;
            color: #ddd
        }

        .help-url {
            margin-top: 20px
        }

        .auth-expl-icon {
            display: block;
            margin-top: 30px;
            font-size: 40px;
            color: #FD9739
        }

        .auth-text {
            margin: auto;
            text-align: center;
            max-width: 300px;
            margin-bottom: 50px;
            margin-top: 30px;
            line-height: 24px
        }

        .bold {
            font-weight: 500
        }
    }
</style>
